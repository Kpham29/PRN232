...
public class BorrowSlipController : Controller {
    ...
    [HttpPost] public async Task<IActionResult> Create(CreateBorrowSlipVm vm) {
        var (ok, body) = await _api.PostAsync("BorrowSlips", new { vm.ReaderId, Books = vm.Items });
    ...
    [HttpPost] public async Task<IActionResult> ApproveRequest(int id, DateTime? borrowedAt, DateTime? dueDate) {
        var (ok, body) = await _api.PutAsync($"BorrowRequests/{id}/Approve", new { BorrowedAt = borrowedAt, DueDate = dueDate });
